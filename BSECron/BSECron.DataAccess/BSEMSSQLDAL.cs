...
namespace BSECron.DataAccess
{
    public class BSEMSSQLDAL
    {
        ...
        public DateTime GetMostRecentDateFromDB()
        {
            DateTime tradingDate;
            using (var scon = new SqlConnection(constring))
            using (var scmd = new SqlCommand("SELECT TOP 1 TRADING_DATE FROM BSEDATA ORDER BY TRADING_DATE DESC", scon))
            {
                ...
                tradingDate = (DateTime)scmd.ExecuteScalar();
                scon.Close();
            }
        ...
        public IEnumerable<Quote> GetHistoricalQuotes(String symbol, DateTime fromDate, DateTime ToDate)
        {
            List<Quote> quotes = new List<Quote>();
            String query = "SELECT *  FROM BSEDATA where SC_NAME = '" + symbol + "' and TRADING_DATE BETWEEN '" +
                fromDate.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture) + "' AND '" +
            ...
            using (var scmd = new SqlCommand(query, scon))
            {
                ...
                while (reader.Read())
                {
                    ...
                    quote.High = Convert.ToDecimal(reader["High"].ToString());
                    quote.Low = Convert.ToDecimal(reader["Low"].ToString());
                    quote.Open = Convert.ToDecimal(reader["Open"].ToString());
                    quote.Volume = Convert.ToDecimal(reader["No_of_Shrs"].ToString());
                    quote.Close = Convert.ToDecimal(reader["close"].ToString());
                    quote.Date = (DateTime)reader["TRADING_DATE"];
                    quotes.Add(quote);
                }
